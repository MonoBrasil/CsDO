    ...
    internal class ResultCache
    {
        ...
        private void Load( )
        {
            lock ( syncRoot )
            {
                ...
                try
                {
                    if ( File.Exists( ConfigurationHelper.New().CacheFile ) )
                    {
                        reader = new StreamReader(ConfigurationHelper.New().CacheFile);
                ...
                catch
                {
                    ...
                    if (File.Exists(ConfigurationHelper.New().CacheFile))
                        File.Delete(ConfigurationHelper.New().CacheFile);
                }
            }
        }
        ...
        private void Save( )
        {
            ...
            lock ( syncRoot )
            {
                TextWriter writer = new StreamWriter(ConfigurationHelper.New().CacheFile, false);
                XmlSerializer serializer = new XmlSerializer( typeof( ResultCacheEntry[ ] ) );
